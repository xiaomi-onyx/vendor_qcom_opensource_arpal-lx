...
struct AidlToLegacy {
    ...
    static void convertPalStreamAttributes(const PalStreamAttributes &aidlConfig, struct pal_stream_attributes *palStreamAttributes);

    static void convertPalDevice(const std::vector<PalDevice> &aidlConfig, struct pal_device *palDevice);

    static void convertModifierKV(const std::vector<ModifierKV> &aidlConfig, struct modifier_kv *modifierKV);

    static void convertPalBufferConfig(const PalBufferConfig &aidlConfig, struct pal_buffer_config *palBufferConfig);

    static void convertPalBuffer(const PalBuffer &aidlConfig, struct pal_buffer *palBuffer, bool externalMemory, bool copyBuffers);

    static void convertPalParamPayload(const PalParamPayload &aidlConfig, pal_param_payload *palParamPayload);

    static void convertPalVolumeData(const PalVolumeData &aidlConfig, pal_volume_data *palVolumeData);
