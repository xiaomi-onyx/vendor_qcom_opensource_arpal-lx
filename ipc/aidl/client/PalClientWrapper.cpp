                        ...
                        pal_stream_handle_t **stream_handle)
{
    ...
    int32_t ret = -EINVAL;
    std::shared_ptr<IPALCallback> ClbkBinder =
                ::ndk::SharedRefBase::make<PalCallback>(cb);
    ...
    auto aidlPalStreamAttr = LegacyToAidl::convertPalStreamAttributes(attr);

    if (devices) {
        for (unsigned long cnt = 0; cnt < no_of_devices; ++cnt) {
            auto aidlPalDev = LegacyToAidl::convertPalDevice(&devices[cnt]);
            aidlDevVec.push_back(std::move(aidlPalDev));
        }
    } else {
        ALOGE("Invalid devices");
    }
    if (modifiers) {
        for (unsigned long cnt = 0; cnt < no_of_modifiers; ++cnt) {
            auto aidlModKV = LegacyToAidl::convertModifierKV(&modifiers[cnt]);
            aidlKvVec.push_back(std::move(aidlModKV));
        }
    } else {
...
int32_t pal_stream_close(pal_stream_handle_t *stream_handle)
{
    ...
    return statusTFromBinderStatus(
        client->ipc_pal_stream_close(
            convertLegacyHandleToAidlHandle(stream_handle)));
}
...
int32_t pal_stream_drain(pal_stream_handle_t *stream_handle, pal_drain_type_t type)
{
    ...
    auto aidlDrainType = LegacyToAidl::convertPalDrainType(type);
    return statusTFromBinderStatus(
                                   ...
                                   pal_buffer_config_t *out_buff_cfg)
{
    ...
    auto aidlInBufCfg = LegacyToAidl::convertPalBufferConfig(in_buff_cfg);
    auto aidlOutBufCfg = LegacyToAidl::convertPalBufferConfig(out_buff_cfg);

    std::vector<PalBufferConfig> _aidl_return_buff_cfg;

    auto status = client->ipc_pal_stream_set_buffer_size((int64_t) stream_handle,
                                                         aidlInBufCfg,
                                                         aidlOutBufCfg,
                                                         &_aidl_return_buff_cfg);
    if (!_aidl_return_buff_cfg.empty())
    {
        in_buff_cfg->buf_count = _aidl_return_buff_cfg[0].buf_count;
        in_buff_cfg->buf_size = _aidl_return_buff_cfg[0].buf_size;
        in_buff_cfg->max_metadata_size = _aidl_return_buff_cfg[0].max_metadata_size;
    }
    if (_aidl_return_buff_cfg.size() == 2)
    {
        out_buff_cfg->buf_count = _aidl_return_buff_cfg[1].buf_count;
        out_buff_cfg->buf_size = _aidl_return_buff_cfg[1].buf_size;
        out_buff_cfg->max_metadata_size = _aidl_return_buff_cfg[1].max_metadata_size;
    }
...
ssize_t pal_stream_read(pal_stream_handle_t *stream_handle, struct pal_buffer *buf)
{
    ...
    if (_aidl_return_buf.ret > 0) {
        if (_aidl_return_buf.buffer.data()->size > buf->size) {
            ...
            if (buf->ts) {
                buf->ts->tv_sec =
                    _aidl_return_buf.buffer.data()->timeStamp.tvSec;
                buf->ts->tv_nsec =
                    _aidl_return_buf.buffer.data()->timeStamp.tvNSec;
            }
            buf->flags = _aidl_return_buf.buffer.data()->flags;
            if (buf->buffer) {
                memcpy(buf->buffer,
                       _aidl_return_buf.buffer.data()->buffer.data(),
                       buf->size);
            }
        }
    }
...
ssize_t pal_stream_write(pal_stream_handle_t *stream_handle, struct pal_buffer *buf)
{
    ...
    auto status = client->ipc_pal_stream_write((int64_t) stream_handle,
                                     aidlPalBufVec, &aidlReturn);
                              ...
                              uint32_t no_of_devices, struct pal_device *devices)
{
    ...
    int32_t cnt = 0;
    uint32_t dev_size = 0;
    std::vector<PalDevice> aidlPalDevVec;

    ALOGD("%s:%d:total device size %d", __func__, __LINE__, dev_size);
    for (cnt = 0; cnt < no_of_devices; ++cnt) {
        auto aidlPalDev = LegacyToAidl::convertPalDevice(&devices[cnt]);
        aidlPalDevVec.push_back(std::move(aidlPalDev));
    }

    return statusTFromBinderStatus(
        client->ipc_pal_stream_set_device(aidlStreamHandle,
                                          aidlPalDevVec));
}
                             ...
                             pal_param_payload **param_payload)
{
    ...
    if (status.isOk()) {
        *param_payload = (pal_param_payload *)calloc(1, sizeof(pal_param_payload) +
                                                            _aidl_return_paramPayload.payload.size());
                             ...
                             pal_param_payload *param_payload)
{
    ...
    PalParamPayload aidlParamPayload = LegacyToAidl::convertPalParamPayload(param_payload);

    auto aidlStreamHandle = convertLegacyHandleToAidlHandle(stream_handle);
    return statusTFromBinderStatus(
        client->ipc_pal_stream_set_param(aidlStreamHandle, param_id,
                                         aidlParamPayload));
}
                              ...
                              struct pal_volume_data *volume)
{
    ...
    auto aidlStreamHandle = convertLegacyHandleToAidlHandle(stream_handle);
    auto aidlVolData = LegacyToAidl::convertPalVolData(volume);

    return statusTFromBinderStatus(client->ipc_pal_stream_set_volume(
            aidlStreamHandle, aidlVolData));
}
...
int32_t pal_get_timestamp(pal_stream_handle_t *stream_handle, struct pal_session_time *stime)
{
    ...
    auto status = statusTFromBinderStatus(client->ipc_pal_get_timestamp(aidlHandle, &aidlSessionTime));
...
int32_t pal_add_remove_effect(pal_stream_handle_t *stream_handle, pal_audio_effect_t effect, bool enable)
{
    ...
    auto aidlAudioEffect = LegacyToAidl::convertPalAudioEffect(effect);
    return statusTFromBinderStatus(client->ipc_pal_add_remove_effect(aidlHandle, aidlAudioEffect, enable));
}
...
int32_t pal_set_param(uint32_t param_id, void *param_payload, size_t payload_size)
{
    ...
    std::vector<uint8_t> aidlPayload;
    aidlPayload.resize(payload_size);
                                      ...
                                      struct pal_mmap_buffer *info)
{
    ...
    PalMmapBuffer aidlBuffer = LegacyToAidl::convertPalMmapBuffer(info);

    return statusTFromBinderStatus(client->ipc_pal_stream_create_mmap_buffer(aidlHandle,
                                                                             min_size_frames,
                                                                             &aidlBuffer));
}
                                     ...
                                     struct pal_mmap_position *position)
{
    ...
    PalMmapPosition aidlPosition = LegacyToAidl::convertPalMmapPosition(position);

    return statusTFromBinderStatus(client->ipc_pal_stream_get_mmap_position(aidlHandle,
                                                                          &aidlPosition));
}
...
int32_t pal_register_global_callback(pal_global_callback cb, uint64_t cookie)
{
    ...
    return statusTFromBinderStatus(client->ipc_pal_register_global_callback(aidlPalCallback, aidlCookie));
}
                         ...
                         pal_stream_type_t pal_stream_type, unsigned int dir)
{
    ...
    return statusTFromBinderStatus(client->ipc_pal_gef_rw_param(aidlParamId,
                                                                aidlPayload,
                                                                aidlPalDeviceId,
                                                                aidlPalStreamType,
                                                                aidlDir,
